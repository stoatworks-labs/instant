...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with four passes this also records *which* one, which is
    ...
    **A pass buffer that could not be allocated.** This plugin holds three
    picture-sized buffers across frames (the captured frame and two halves of
    the development state), and the failure mode when the driver says no is
    a black frame with nothing to explain it. What goes in the log is the
    size asked for, and the size the plugin settled on after every resize.
*/
namespace instant::diag
{
