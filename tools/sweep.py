...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/intest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**Time controls need time.** Speed, Temperature and Interval only mean
anything once film-seconds have passed: the sweep renders 40 frames of the
moving card at a synthetic 60 fps, which at the default 64x is 42 s of
development -- the print is half-developed and every chemistry control shows.

**Take is an event, and it needs Take mode.** Its context sets `Mode` to
Take. At 0 no press ever comes and the plugin is a viewfinder (the clip); at
1 the press is delivered once through `--set`, before the first frame, which
is what a host does with a button, and the print develops. In Continuous mode
(the default) the first frame takes on its own, so the press would change
nothing.

**Interval needs a retake inside the render.** At 0.5 s the Continuous mode
takes again at frame 30, from a card that has moved; at 128 s it never does.

**Mode's ends are Take (the viewfinder, with no press) and Continuous (a
print).**

**An option's range is its element count.** `intest --list` prints it for
...
CONTEXT = {
    # Take mode (index 0), with the press delivered once before the first
    # frame at 1 and never at 0.
    "Take": {"Mode": 0},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "intest"))
    ...
    scratch = tempfile.mkdtemp(prefix="insweep")
